import { FlatList, RefreshControl, ScrollView, useColorScheme, View, Keyboard, Animated, Easing } from "react-native";
...
export default function ListMomentsWithoutInMemory() {
    ...
    const addMomentsFunc = async () => {
        if(selectedMoments.length > 0) { await addMoments() }
    }
    const fetchData = async () => {
        async function getMoments() {           
            try{
                await api.post(`/moment/get-user-moments/tiny/exclude-memory`, {
    ...
    if(networkStats == 'OFFLINE' && allMoments.length == 0) return <OfflineCard height={(sizes.screens.height - sizes.headers.height) / 2}/>
        ...
        <Animated.View style={container}>
            <View style={header}>
                ...
                <View style={{opacity: selectedMoments.length > 0? 1 : 0.3}}>
                    <ViewMorebutton text={t('Add')} icon={
                        <AddIcon style={{top: 0.5}} width={12} height={12} fill={colors.blue.blue_05.toString( )}/>
