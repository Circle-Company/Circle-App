...
export function EditMemoryProvider({children}: EditMemoryProviderProps) {
    ...
    const [ title, setTitle ] = React.useState(memory.title? memory.title : t("memory title") + "...")
    ...
    async function addMoments() {
        const moments_ids_list = selectedMoments.map((i: any) => { return {id: i.id}})
        try {
    ...
    async function editTitle() {
        try {
            await api.post(`/memory/edit/title`, {
    ...
    async function deleteMemory() {
        try {
            const isAuthenticated = await TouchID.requestBioAuth(t("Make sure it's you"), t("You're deleting this memory"))
            if(isAuthenticated) {
                await api.post(`/memory/delete`, {
                    user_id: session.user.id,
                    memory_id: memory.id,
                })
                .then(() => {
    ...
    return (
        <EditMemoryContext.Provider value={contextValue}>
            {children}
        </EditMemoryContext.Provider>
    )
}
