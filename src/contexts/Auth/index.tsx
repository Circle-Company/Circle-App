...
function normalizeSessionPayload(payload: any): SessionDataType {
    ...
    const preferences = {
        ...
        content: {
            disableAutoplay: Boolean(preferencesPayload.disableAutoplay),
            disableHaptics: Boolean(preferencesPayload.disableHaptics),
            disableTranslation: Boolean(preferencesPayload.disableTranslation),
            muteAudio: Boolean(preferencesPayload.muteAudio),
        },
        pushNotifications: {
            disableLikeMoment: Boolean(preferencesPayload.disableLikeMomentPushNotification),
            disableNewMemory: Boolean(preferencesPayload.disableNewMemoryPushNotification),
            disableAddToMemory: Boolean(preferencesPayload.disableAddToMemoryPushNotification),
            disableFollowUser: Boolean(preferencesPayload.disableFollowUserPushNotification),
            disableViewUser: Boolean(preferencesPayload.disableViewUserPushNotification),
        },
    }
...
const persistSessionData = (session: SessionDataType) => {
    try {
        ...
        safeSet(userKey.verifyed, session.user.isVerified)
        safeSet(userKey.verifyed + ":active", session.user.isActive)
        ...
        safeSet(accountKey.unreadNotificationsCount, session.account.unreadNotificationsCount)
        ...
        safeSet(preferencesKey.appTimezone, session.preferences.appTimezone)
        safeSet(preferencesKey.timezoneCode, session.preferences.timezoneCode)
